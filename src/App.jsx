...
const [buttonHeldDown, setButtonHeldDown] = useState(false)
  ...
  /* Challenge
	...
	Bu sanal eğlence parkı oyunu, korkunç bir bilgisayar korsanı iki state'ini ve dört event handler'ını sildiği için bozuldu.
   Sizin göreviniz aşağıdakileri yaparak oyunu düzeltmek: 
	
		1. Kullanıcının imleci "Basılı Tut" butonunun içine girerse ve mouse butonuna basarsa, eksik olan iki state'in değerleri buna göre değişmelidir.
     Bu durum değişiklikleri gerçekleşirse, aşağıdaki try bloğunun useEffect'indeki mevcut kod, zamanlayıcı ve termometrenin yükselmeye başlamasını
      sağlamalıdır.
		   
		2. Kullanıcının imleci butondan çıkarsa veya mouse butonunu bırakırsa, iki eksik state'in değerleri buna göre değişmelidir.
		   
		3. Tüm bunlar, eksik olay işleyicileri tarafından tetiklenen eksik state değerlerindeki değişiklikler yoluyla gerçekleştirilmelidir.
     Bu eksik state'ler ve olay işleyicilerinin yanında herhangi bir kodu SİLMEMELİ, eklememeli veya değiştirmemelisiniz.
      Bunu doğru şekilde yaparsanız aşağıdaki try bloğu ve useEffect zaten yazılmış oldukları için mükemmel şekilde çalışacaktır. 
...
*/
  ...
  const handleMouseEnterLeave = (isEntering) => {
    setCursorInButton(isEntering);
    if(!isEntering){
      setButtonHeldDown(false)
    }
  }
  
  const handleMouseDownUp = (isDown) =>{
    if(cursorInButton){
      setButtonHeldDown(isDown)
    }
  }
  ...
  return (
    <div className='wrapper'>
      ...
      onMouseDown={()=> handleMouseDownUp(true)}
      onMouseUp={()=> handleMouseDownUp(false)}
      >Basılı Tut</button>
